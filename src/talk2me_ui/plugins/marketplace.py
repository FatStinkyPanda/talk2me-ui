...
class PluginMarketplace:
    ...
    def __init__(self, marketplace_url: str, plugins_dir: Path, cache_dir: Path | None = None):
    ...
    async def list_available_plugins(
        ...
        try:
            async with self.session.get(
                f"{self.marketplace_url}/api/plugins", params=params
            ) as response:
    ...
    async def install_plugin(self, plugin_id: str, version: str | None = None) -> bool:
        ...
        try:
            ...
            if not download_url:
                logger.error(
                    f"No download URL found for plugin {plugin_id} version {target_version}"
                )
                return False
    ...
    async def update_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            if await self.install_plugin(plugin_id, latest_version):
                ...
                shutil.rmtree(plugin_path)
                logger.info(
                    f"Successfully updated plugin {plugin_name} to version {latest_version}"
                )
                return True
            else:
                return False
    ...
    async def _download_and_install_plugin(
        self, download_url: str, plugin_id: str, version: str
    ) -> bool:
