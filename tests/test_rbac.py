...
class TestRBACManager:
    ...
    def test_check_permission_admin_role(self):
        ...
        with patch.object(self.rbac, '_get_role_permissions', return_value={'stt:use', 'system:admin'}):
    ...
    def test_check_any_permission(self):
        ...
        with patch.object(self.rbac, '_get_role_permissions', return_value={'stt:use', 'tts:use'}):
            ...
            assert self.rbac.check_any_permission('user-role-id', [('stt', 'use'), ('tts', 'use')]) is True
            # Test with mixed permissions
            assert self.rbac.check_any_permission('user-role-id', [('stt', 'use'), ('system', 'admin')]) is True
            # Test with no matching permissions
            assert self.rbac.check_any_permission('user-role-id', [('system', 'admin'), ('roles', 'manage')]) is False
...
class TestRBACInitialization:
    ...
    def test_initialize_default_roles_and_permissions(self):
        ...
        with patch('talk2me_ui.rbac.db_permission_manager') as mock_perm_mgr, \
